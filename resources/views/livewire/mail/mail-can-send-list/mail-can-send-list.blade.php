<div class="p-4">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div class="relative w-full sm:w-96">
            ...
            <input type="text" wire:model.defer="search" placeholder="{{ __('Search mail...') }}" class="w-full rounded-xl border border-gray-300 dark:border-gray-700 
            ...
            <button wire:click="applySearch" class="absolute inset-y-0 right-0 bg-blue-600 hover:bg-blue-700 
            ...
            <tr class="hover:bg-gray-50 dark:hover:bg-gray-800">
                ...
                <td class="px-6 py-4 text-center align-middle">
                    <div class="flex flex-col sm:flex-row justify-center gap-2">
                        ...
                        <a href="{{ route('admin.mailcanclientsend.edit', $item)}}"
    ...
    @if($showModal && $selectedMailCanSend)
        <div class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50 p-4" role="dialog"
            aria-modal="true" aria-labelledby="mail-modal-title">

            <div class="bg-white dark:bg-gray-800 rounded-lg w-full max-w-lg overflow-hidden shadow-lg" role="document">

                {{-- Header --}}
                <div class="flex justify-between items-center p-6 border-b border-gray-200 dark:border-gray-700">
                    <h2 id="mail-modal-title" class="text-lg font-semibold text-gray-900 dark:text-gray-100">
                        {{ $selectedMailCanSend->title ?? __('Mail details') }}
                    </h2>

                    <button wire:click="closeMailModal" aria-label="{{ __('Close') }}"
                ...
                <div class="p-6 space-y-4 text-sm text-gray-700 dark:text-gray-300">
                    ...
                    @php
                        $limit = $selectedMailCanSend->mail_limit ?? null;
        ...
        <p>
                        <strong>{{ __('Mail limit') }}:</strong>
                        <span style="color: {{ $color }}">
                            @if ($limit < 1)
                                {{ __('Unavailable units') }}
                            @else
                                {{ $limit }}
                                {{ trans_choice('unit|units', $limit) }}
                            @endif
                        </span>
                    </p>
                ...
                <div class="flex justify-end gap-3 p-6 border-t border-gray-200 dark:border-gray-700">
                    <button wire:click="closeMailModal" class="px-4 py-2 bg-gray-200 dark:bg-gray-700
